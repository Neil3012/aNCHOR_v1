...
namespace Google.XR.ARCoreExtensions.Samples.PersistentCloudAnchors
{
    ...
    public class ARViewManager : MonoBehaviour
    {
        ...
        public GameObject CloudAnchorPrefabV2;
        ...
        private int _anchorCounter = 0;

        public List<Vector3> lineObjects = new List<Vector3>();
        public List<TextMeshPro> textMeshProUGUIs = new List<TextMeshPro>();
        ...
        private IEnumerator ResolveAnchor(string cloudId, ResolveCloudAnchorPromise promise)
        {
            ...
            _anchorCounter++; // Increment the counter for each resolved anchor
            result.Anchor.gameObject.name = "Anchor" + _anchorCounter;
            if (result.CloudAnchorState == CloudAnchorState.Success)
            {
                OnAnchorResolvedFinished(true, cloudId);
                lineObjects.Add(Instantiate(CloudAnchorPrefabV2, result.Anchor.transform).transform.position);
            }
            else
            {
                OnAnchorResolvedFinished(false, cloudId, result.CloudAnchorState.ToString());
            }
            //lineRenderer.positionCount = lineObjects.Count;
            //for (int i = 0; i < lineObjects.Count; i++)
            //{
            //    lineRenderer.SetPosition(i, lineObjects[i]);
            //    Debug.Log("Line I " + i);
            //}
        }
        public void OnButtonDrawClick()
        {
            ...
            {
                textMeshProUGUIs[i].text = (Vector3.Distance(myLocation.transform.position, lineObjects[i])).ToString();
            }
            _anchorCounter = 0;
        }
